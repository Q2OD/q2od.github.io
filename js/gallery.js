...
const { db, verifyPassword, isGalleryExpired } = window.firebaseInit;
...
passwordForm.addEventListener('submit', async (e) => {
  ...
  try {
    ...
    currentGallery = galleryDoc.data();

    // Check if active
    if (!currentGallery.isActive) {
    ...
    // Check if expired
    if (isGalleryExpired(currentGallery.expiresAt)) {
    ...
    // Verify password
    const isValid = await verifyPassword(password, currentGallery.password);
...
async function loadGallery() {
  try {
    // Update header
    galleryTitle.textContent = currentGallery.clientName;
    gallerySubtitle.textContent = currentGallery.eventName;
...
function createMediaItem(media, index) {
  ...
  if (media.type === 'video') {
    item.innerHTML = `
      <video src="${media.storageUrl}" class="pointer-events-none"></video>
      <div class="video-badge">▶ VIDEO</div>
    `;
  } else {
    item.innerHTML = `
      <img src="${media.storageUrl}" alt="${media.filename}" loading="lazy">
    `;
  }
